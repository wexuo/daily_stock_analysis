...
export const StockBar: React.FC<StockBarProps> = ({
  ...
  const selectedCount = [...selectedCodes].filter((code) =>
    deletableItems.some((item) => item.stockCode === code),
  ).length;
  const allVisibleSelected =
    deletableItems.length > 0 && selectedCount === deletableItems.length;
  ...
  return (
    <aside className={`glass-card overflow-hidden flex flex-col ${className}`}>
      <ScrollArea viewportClassName="p-4" testId="home-stock-bar-scroll">
          ...
          {items.length > 0 && onDeleteStock && (
            <div className="flex items-center justify-between gap-2">
              ...
              <Button
                ...
                className="disabled:!border-transparent disabled:!bg-transparent w-6 p-0"
        ...
        {isLoading ? (
          ...
          <div className="space-y-1.5">
            {items.map((item) => {
              ...
              const isSelected =
                selectedRecordId === item.id || selectedStockCode === code;
