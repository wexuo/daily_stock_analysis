...
const PortfolioPage: React.FC = () => {
  ...
  const [positionAnalysisLoadingKey, setPositionAnalysisLoadingKey] = useState<
    string | null
  >(null);
  const [positionAnalysisMessage, setPositionAnalysisMessage] = useState<
    string | null
  >(null);
  ...
  const [pendingAccountDelete, setPendingAccountDelete] =
    useState<PendingAccountDelete | null>(null);
  ...
  const loadAccounts = useCallback(async () => {
    try {
      ...
      if (items.length === 0) setShowCreateAccountModal(false);
    } catch (err) {
  ...
  const handleConfirmAccountDelete = async () => {
    ...
    try {
      ...
      const nextAccount = accounts.find(
        (item) => item.id !== pendingAccountDelete.accountId,
      );
      setSelectedAccount(nextAccount?.id ?? "all");
      setPendingAccountDelete(null);
      setShowCreateAccountModal(!nextAccount);
      await loadAccounts();
      setEventPage(1);
    } catch (err) {
      setError(getParsedApiError(err));
    ...
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      ...
      >
        {accountCreateError ? (
          <InlineAlert
            variant="danger"
            className="mb-3 rounded-lg px-2 py-1 text-xs shadow-none"
            title="创建账户失败"
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        ...
        <Card padding="md">
          ...
          <div className="mt-3 text-xs text-secondary space-y-1">
            ...
            <div>
              {text.sectorAlert}:{" "}
              {risk?.sectorConcentration?.alert ? text.yes : text.no}
            </div>
            <div>
              {text.topWeight}:{" "}
              {formatPct(
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-2">
            {text.drawdownMonitor}
          </h3>
          <div className="text-xs text-secondary space-y-1">
            <div>
              {text.maxDrawdown}: {formatPct(risk?.drawdown?.maxDrawdownPct)}
            </div>
            <div>
              {text.currentDrawdown}:{" "}
              {formatPct(risk?.drawdown?.currentDrawdownPct)}
            </div>
            <div>
              {text.alert}: {risk?.drawdown?.alert ? text.yes : text.no}
            </div>
          </div>
        </Card>
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-2">
            {text.stopLossWarning}
          </h3>
          <div className="text-xs text-secondary space-y-1">
            <div>
              {text.triggeredCount}: {risk?.stopLoss?.triggeredCount ?? 0}
            </div>
            <div>
              {text.nearCount}: {risk?.stopLoss?.nearCount ?? 0}
            </div>
            <div>
              {text.alert}: {risk?.stopLoss?.nearAlert ? text.yes : text.no}
            </div>
          </div>
        </Card>
        <Card padding="md">
          ...
          <div className="text-xs text-secondary space-y-1">
            <div>
              {text.accountCount}: {snapshot?.accountCount ?? 0}
            </div>
            <div>
              {text.currency}: {snapshot?.currency || "CNY"}
            </div>
            <div>
              {text.costMethodShort}:{" "}
              {(snapshot?.costMethod || costMethod).toUpperCase()}
            </div>
          </div>
        </Card>
      </section>
