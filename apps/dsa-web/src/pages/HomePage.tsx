...
const HomePage: React.FC = () => {
  ...
  const [marketReviewPayload, setMarketReviewPayload] =
    useState<MarketReviewPayload | null>(null);
  const [analysisSkills, setAnalysisSkills] = useState<SkillInfo[]>([]);
  const [selectedStrategyId, setSelectedStrategyId] = useState("");
  const [strategyMenuOpen, setStrategyMenuOpen] = useState(false);
  const [runFlowDrawer, setRunFlowDrawer] = useState<RunFlowDrawerState>({
    open: false,
  });
  ...
  const liveMarketReviewLanguage = normalizeReportLanguage(
    marketReviewPayload?.language,
  );
  const isMarketReviewHistoryReport =
    selectedReport?.meta.reportType === "market_review";
  const isHistoryTrendUnavailable =
    !selectedReport || !selectedReport.meta.stockCode;
  ...
  const strategyOptions = useMemo(
    () => [
      {
        id: "",
        name: t("home.defaultStrategyName"),
        description: t("home.defaultStrategyDescription"),
      },
  ...
  const focusStrategyItem = useCallback(
    (index: number) => {
  ...
  const getSelectedStrategyIndex = useCallback(() => {
    const selectedIndex = strategyOptions.findIndex(
      (option) => option.id === selectedStrategyId,
    );
    return selectedIndex >= 0 ? selectedIndex : 0;
  }, [selectedStrategyId, strategyOptions]);
  useEffect(() => {
    strategyItemRefs.current = strategyItemRefs.current.slice(
      0,
      strategyOptions.length,
    );
  }, [strategyOptions.length]);
  useEffect(() => {
    ...
    const targetIndex =
      strategyInitialFocusIndexRef.current ?? getSelectedStrategyIndex();
  ...
  const handleStrategyButtonKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLButtonElement>) => {
      ...
      const targetIndex =
        event.key === "ArrowUp" ? strategyOptions.length - 1 : 0;
      if (strategyMenuOpen) {
  ...
  const handleStrategyMenuKeyDown = useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      const currentIndex = strategyItemRefs.current.findIndex(
      ...
      switch (event.key) {
        ...
        case "ArrowUp":
          event.preventDefault();
          focusStrategyItem(
            currentIndex >= 0 ? currentIndex - 1 : itemCount - 1,
          );
          break;
        case "Home":
          event.preventDefault();
  ...
  useEffect(() => {
    ...
    const stockCode =
      typeof state?.stockCode === "string" ? state.stockCode.trim() : "";
    if (!stockCode) {
      return;
    }
    const stockName =
      typeof state?.stockName === "string" ? state.stockName.trim() : "";
    setQuery(stockCode);
  ...
  const openHistoryRunFlow = useCallback(
    (recordId: number) => {
      const meta =
        selectedReport?.meta.id === recordId ? selectedReport.meta : null;
  ...
  const mergedStockBarItems = useMemo<StockBarItem[]>(() => {
    ...
    const stockItems = stockBarItems.filter(
      (item) => item.stockCode !== "MARKET",
    );
    ...
    return [marketReviewItem, ...stockItems].sort((left, right) => {
      const leftTime = left.lastAnalysisTime
        ? Date.parse(left.lastAnalysisTime)
        : 0;
      const rightTime = right.lastAnalysisTime
        ? Date.parse(right.lastAnalysisTime)
        : 0;
              ...
              {analysisSkills.length > 0 ? (
                <div ref={strategyMenuRef} className="relative flex-shrink-0">
                  <button
                    ...
                    aria-controls={
                      strategyMenuOpen ? "strategy-menu" : undefined
                    }
                  ...
                  >
                    ...
                    <span className="truncate">
                      {selectedStrategy?.name || t("home.strategy")}
                    </span>
                  </button>
                  {strategyMenuOpen ? (
                    <div
                    ...
                    >
                      {strategyOptions.map((option, index) => {
                        ...
                        return (
                          ...
                          >
                            <Check
                              className={`mt-0.5 h-4 w-4 flex-shrink-0 ${selected ? "opacity-100" : "opacity-0"}`}
                              aria-hidden="true"
                            />
        ...
        {setupNeedsAction ? (
          <div className="px-3 pb-2 md:px-4">
            <InlineAlert
              ...
              message={
                setupMissingLabels
                  ? t("home.setupMissingWithLabels", {
                      labels: setupMissingLabels,
                    })
        ...
        <div className="flex-1 flex min-h-0 overflow-hidden">
          ...
          >
            ...
            {!marketReviewReport && isLoadingReport ? (
              ...
              <div
                className={
                  isHistoryTrendOpen ? "space-y-4 pb-8" : "space-y-4 pb-8"
                }
              >
